...
namespace InstagramTools.WebApi
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
      {
        ...
        string connection = this.GetConnectionStringForMachine(this.Configuration);
        ...
        services.AddAuthorization(
          options =>
            {
              options.AddPolicy(
                Constants.Policies.UserMinimum,
                policy => policy.RequireClaim(
                  Constants.ClaimTypes.RoleName,
                  Constants.Roles.User,
                  Constants.Roles.Moderator,
                  Constants.Roles.Admin));

              options.AddPolicy(
                Constants.Policies.ModeratorMinimum,
                policy => policy.RequireClaim(
                  Constants.ClaimTypes.RoleName,
                  Constants.Roles.Moderator,
                  Constants.Roles.Admin));
              ...
              options.AddPolicy(
                Constants.Policies.AdminOnly,
                policy => policy.RequireClaim(Constants.ClaimTypes.RoleName, Constants.Roles.Admin));
            });
      ...
      // This method gets called by the runtime. Use this method to configure the HTTP request pipeline
        ...
        private void ConnectServices(IServiceCollection services)
        {
            ...
            services.Configure<JwtIssuerOptions>(options =>
            {
                ...
                options.SigningCredentials = new SigningCredentials(this._signingKey, SecurityAlgorithms.HmacSha256);
            });
